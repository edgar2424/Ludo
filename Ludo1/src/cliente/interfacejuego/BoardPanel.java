...
public class BoardPanel extends JPanel implements ObservadorIF {
	...
	private BoardPanel(){
		this.p = Main.tamanoPixel;
		this.altura = p*15; 
		this.largura = p*15;
		GameFacade.GetjuegoFacade().add(this);
		SetProperties();
	...
	private void DrawPlayersGamepiezas( Graphics2D g2d){
		if(this.piezas !=null){
			int countColor = 0;
			g2d.setStroke(new BasicStroke(2));
			for(int[][] piezas : this.piezas/*new int[][][] {juegof.GetRedpiezas(), juegof.GetBluepiezas(), juegof.GetGreenpiezas(), juegof.GetYellowpiezas()}*/){
				for(int piece = 0; piece<4;piece++)
					DrawPiece(g2d, countColor, piezas, piece);
				countColor++;
			}	
		}
	}

	private void DrawPiece(Graphics2D g2d, int count, int[][] piezas, int piece) {
		SetpiezasPaint(g2d, count);
		Ellipse2D ec = new Ellipse2D.Double(piezas[piece][0]*p,piezas[piece][1]*p, p,p);
		FillAndDrawBlackLine(g2d,ec);
		SetPaint(g2d, count);
		ec = new Ellipse2D.Double(piezas[piece][0]*p + p/6,piezas[piece][1]*p+ p/6 , p-p/3,p-p/3);
		FillAndDrawBlackLine(g2d,ec);
	}
	...
	private void SetpiezasPaint(Graphics2D g2d, int i) {
		switch (i){
